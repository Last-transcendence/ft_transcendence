...
const CreatOrModifyBody = ({
	...
	const [twoFATrueFalse, setTwoFATrueFalse] = useState<string>('false');
	...
	const handleChangePicture = (e: ChangeEvent<HTMLInputElement>) => {
		...
		if (file) {
			...
			reader.onerror = error => {
				setServerErrorMessage('파일을 읽는것을 실패했습니다');
			};
	...
	const handleToggleTwoFactorState = () => {
		setTwoFATrueFalse(twoFATrueFalse === 'true' ? 'false' : 'true');
		if (twoFATrueFalse === 'true') {
	...
	const Mdify2FaValue: ModifyTwoFactorProps = {
		checked: twoFATrueFalse === 'true' ? true : false,
	...
	return isModify === true && !me ? (
		<Loading />
	) : (
		<Stack justifyContent={'space-between'} height="70%">
			<Container maxWidth="xs">
				<CustomSnackbar
					open={serverErrorMessage !== '' ? true : false}
